...
// Half-duplex only: the peripheral shares one line with the central, so it may
// only drive TX in the window after a poll. begin_tx is therefore deferred and
// debounced against the incoming poll. In full-duplex the peripheral owns its TX
// line and sends immediately from split_peripheral_wired_report_event (its
// begin_tx()), so none of this turnaround machinery is needed.
    ...
    while (ring_buf_size_get(&chosen_rx_buf) > MSG_EXTRA_SIZE) {
        ...
        case 0:
            ...
            if (env.payload.cmd.type == ZMK_SPLIT_TRANSPORT_CENTRAL_CMD_TYPE_POLL_EVENTS) {
                // Respond to the central's poll so it (and we) stay healthy: emit a
                // heart-beat if we have been quiet, which also advances the central's
                // last-response time. In full-duplex the heart-beat's report_event
                // sends immediately; only half-duplex needs the debounced turnaround.
